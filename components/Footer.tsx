      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="border-t border-gray-800 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex space-x-6 mt-4 md:mt-0">
            <a href={theme.socialMedia?.facebook || '#'} target="_blank" rel="noopener noreferrer">
              <Facebook className="h-5 w-5 text-gray-400 hover:text-white cursor-pointer" />
            </a>
            <a href={theme.socialMedia?.instagram || '#'} target="_blank" rel="noopener noreferrer">
              <Instagram className="h-5 w-5 text-gray-400 hover:text-white cursor-pointer" />
            </a>
            {theme.socialMedia?.twitter && (
              <a href={theme.socialMedia.twitter} target="_blank" rel="noopener noreferrer">
                <Twitter className="h-5 w-5 text-gray-400 hover:text-white cursor-pointer" />
              </a>
            )}
            {theme.socialMedia?.linkedin && (
              <a href={theme.socialMedia.linkedin} target="_blank" rel="noopener noreferrer">
